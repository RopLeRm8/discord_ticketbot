...
export default async function buttonsHandler(interaction: Interaction) {
  if (!interaction.isButton() || !interaction) return;
  ...
  switch (customId) {
    case "close_ticket":
      if (
        ...
        const intervalId = setInterval(async () => {
          if (counter > 0) {
            await interaction.editReply(
              `Closing ticket in ${counter--} seconds...`
            );
          } else {
        ...
        try {
          ...
          console.log(
            "Updated Permissions:",
            channel.permissionOverwrites.cache.map((p) => p.toJSON())
          );
        } catch (error) {
    ...
    case "claim_ticket":
      if (channel && isAdmin && !isTicketCreator && !isClaimed) {
        ...
        if (isClaimed) message = "This ticket has already been claimed.";
